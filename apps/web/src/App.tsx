...
export default function App() {
  const store = useCodexStore(); const [query, setQuery] = useState("");
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">{store.error && <div className="error-banner">{store.error}<button onClick={() => useCodexStore.setState({ error: undefined })}>×</button></div>}{store.active ? <CodexRuntimeProvider messages={toAssistantMessages(store.active.turns)} isRunning={activeRunning} onSend={store.send} onCancel={store.interrupt}><header className="thread-header"><div><h1>{store.active.name ?? (store.active.preview || "Untitled thread")}</h1><span>{store.active.cwd}</span></div><div><button onClick={() => { const name = prompt("Thread name", store.active?.name); if (name && store.active) void api.rename(store.active.id, name).then(() => store.refresh()); }}>Rename</button><button onClick={() => store.active && void api.archive(store.active.id).then(() => store.loadThreads())}>Archive</button></div></header><div className="content-scroll"><AssistantThread turns={store.active.turns} /><Activity turns={store.active.turns} /></div>{store.approvals.filter((item) => item.threadId === store.active?.id).map((item) => <ApprovalCard key={item.requestId} approval={item} />)}<AssistantComposer /></CodexRuntimeProvider> : <div className="empty"><div className="logo large">C</div><h1>Continue your Codex work</h1><p>Select a native CLI thread or start a new one.</p></div>}</section>
  </main>;
}
